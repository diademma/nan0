...
/**
 * Итоговая стабильная версия MainActivity.
 * Ждет полной инициализации стартового роутера Gmail Touch (#tl/priority/...)
 * перед тем как разрешить отправку или запустить автоматический тест.
 */
public class MainActivity extends AppCompatActivity {
    ...
    private boolean initialLoadFinished = false; // Флаг готовности роутера Gmail
    ...
    private void handleWorkerPageFinished(String url) {
        ...
        // Ждем, пока Gmail Touch полностью инициализирует свой стартовый роутер и откроет домашний экран (#tl/...)
        if (!initialLoadFinished && url != null && (url.contains("#tl/") || url.contains("priority") || url.contains("Inbox"))) {
            initialLoadFinished = true;
            logStep("WORKER: Инициализация Gmail завершена! Роутер стабилен.");
            
            // Активируем кнопку отправки
            runOnUiThread(() -> btnSend.setEnabled(true));

            // Безопасно запускаем автотест через 1.5 секунды после стабилизации
            handler.postDelayed(() -> {
    ...
    private void onSendClicked() {
        if (!initialLoadFinished) {
            logStep("SEND_ERR: Gmail ещё инициализируется, подождите...");
            return;
        }
    ...
    private void unlockSession() {
        ...
        // Убираем слепой 3-секундный таймер автоматического теста.
        // Запуск перенесен в handleWorkerPageFinished() после полной готовности роутера Gmail.
    ...
    private void showChatScreen() {
        ...
        // Держим кнопку выключенной, пока Gmail полностью не инициализируется
        btnSend.setEnabled(initialLoadFinished);
    }
