 ...
 * Использует технику Cache-Buster (?r=timestamp) для принудительного полного сброса
 * роутера Gmail Touch, избегая бесконечных циклов перенаправления на страницу создания (#co).
 */
public class MainActivity extends AppCompatActivity {
    ...
    private void injectFillCompose(String text) {
        ...
        String js = "javascript:(function(){"
                ...
                // 4. Имитация клика по кнопке «Назад» (id="cmcb") для штатного сохранения в Gmail Touch
        ...
        workerWebView.evaluateJavascript(js, new ValueCallback<String>() {
            ...
            public void onReceiveValue(String value) {
                ...
                // Даем Gmail 2.5 секунды на отправку запроса и завершение фоновых анимаций сохранения
                handler.postDelayed(() -> {
                    logStep("SAVE: Проверяю появление сохраненного черновика в списке...");
                    verifySavedDraft();
                }, 2500L);
            }
        });
    ...
    private void verifySavedDraft() {
        ...
        // Используем трюк Cache-Buster (?r=timestamp) для форсирования ПОЛНОЙ перезагрузки страницы.
        // Это полностью ломает внутренний роутер Gmail, сбрасывает зависший Compose и заставляет загрузить чистую папку.
    ...
    private void pollDrafts() {
        ...
        // Принудительно запрашиваем свежий список черновиков с сервера через полную перезагрузку
        workerWebView.loadUrl(dynamicBaseUrl + "?r=" + System.currentTimeMillis() + "#tl/Drafts");
