...
/**
 * Отладочная версия MainActivity.
 * Все технические шаги пишутся прямо в интерфейс чата.
 * После успешной авторизации автоматически инициирует создание тестового черновика.
 */
public class MainActivity extends AppCompatActivity {
    ...
    // Множество отправленных нами писем, чтобы отличать их от входящих ответов сервера
    ...
    private void setupLoginWebView() {
        ...
        loginWebView.setWebViewClient(new WebViewClient() {
            ...
            public void onReceivedError(WebView view, WebResourceRequest request, WebResourceError error) {
                ...
                // Отсекаем логи ошибок заблокированных фаерволом фоновых доменов Google (вроде signaler-pa)
    ...
    private void handleWorkerPageFinished(String url) {
        // Запоминаем динамический адрес мобильной Touch-сессии
    ...
    private void createDraftToSelf(String text) {
        ...
        // Сразу отображаем отправленное сообщение в чате
        pushMessage(text, true, null);

        // Загружаем форму создания письма в Touch-интерфейсе
        workerWebView.post(() -> workerWebView.loadUrl(dynamicBaseUrl + "#co"));

        // Дублирующий таймер для надежности, если переход по хэшу (#co) не вызовет onPageFinished
        handler.postDelayed(this::tryFillCompose, 2500L);
    }
    ...
    private void tryFillCompose() {
        ...
        logStep("SEND: эмулирую заполнение полей письма...");
    ...
    private void injectFillCompose(String text) {
        ...
        String js = "javascript:(function(){"
                ...
                + "}"
                + "return (toEl?'TO_OK':'TO_ERR')+' '+(subEl?'SUB_OK':'SUB_ERR')+' '+(bodyEl?'BODY_OK':'BODY_ERR');"
                + "})();";

        workerWebView.evaluateJavascript(js, new ValueCallback<String>() {
            @Override
            public void onReceiveValue(String value) {
                logStep("SEND: поля заполнены (" + stripJsValue(value) + ")");
                op = Op.WAITING_SAVE;

                // Даем Gmail немного времени зафиксировать ввод перед выходом со страницы
                handler.postDelayed(() -> {
    ...
    private void verifySavedDraft() {
        ...
        // Возврат в список черновиков инициирует сохранение
            ...
            public void onReceiveValue(String value) {
                ...
                try {
                    ...
                    for (int i = 0; i < arr.length(); i++) {
                        ...
                        // Игнорируем наши собственные отправленные черновики, чтобы не дублировать их
    ...
    private String buildDraftSearchJs(String searchStr, boolean exactMatch) {
        ...
        return "javascript:(function(){"
                ...
                + "var res=[];"
                // 1. Поиск по Touch-структуре [data-thread-id]
    ...
    private void unlockSession() {
        ...
        // СРАЗУ ПОСЛЕ АВТОРИЗАЦИИ: запускаем автоматическую отправку тестового черновика через 3 секунды
        handler.postDelayed(() -> {
